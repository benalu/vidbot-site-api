...
func (h *Handler) Extract(c *gin.Context) {
	stats.Platform(c, "content", "tiktok")
	var req Request
	...
	if !validator.IsValidURL(req.URL) || !validator.IsAllowedDomain(req.URL, "tiktok") {
		response.InvalidURLWarn(c, "content", "tiktok", req.URL)
		return
	}
	...
	if cached, err := downloader.CacheGet[mediaresponse.TikTokResponseNew]("content", "tiktok", req.URL); err == nil && cached != nil {
		for i, v := range cached.Download.Video {
			...
			cached.Download.Video[i].Server2 = downloader.GenerateServer2URL(
				h.appURL, h.streamSecret, downloader.CacheKey("content", "tiktok", v.Original),
				v.Original, customTitle, "", "", ext, "content",
			)
		}
		if cached.Download.Audio != nil {
			...
			cached.Download.Audio.Server2 = downloader.GenerateServer2URL(
				h.appURL, h.streamSecret, downloader.CacheKey("content", "tiktok", cached.Download.Audio.Original),
				cached.Download.Audio.Original, audioTitle, "", "", "mp3", "content",
			)
		}
	...
	if err != nil {
		response.Extraction(c, "content", "tiktok", err)
		return
	}
	...
	for _, v := range result.Videos {
		...
		server2 := downloader.GenerateServer2URL(
			h.appURL, h.streamSecret, downloader.CacheKey("content", "tiktok", v.URL),
			v.URL, customTitle, "", "", ext, "content",
		)
	...
	if result.AudioURL != "" {
		...
		audio = &mediaresponse.ContentAudio{
			...
			Server2: downloader.GenerateServer2URL(
				h.appURL, h.streamSecret, downloader.CacheKey("content", "tiktok", result.AudioURL),
				result.AudioURL, audioTitle, "", "", audioExt, "content",
			),
		}
	}
