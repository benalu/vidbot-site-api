...
func (h *Handler) Convert(c *gin.Context) {
	...
	if verr := convertvalidator.Validate(req.URL, convertvalidator.Fonts); verr != nil {
		stats.TrackError(c, "convert", "fonts", verr.Code)
	...
	if err != nil {
		slog.Error("conversion failed", "group", "convert", "category", "fonts", "error", err)
		stats.TrackError(c, "convert", "fonts", "CONVERT_ERROR")
	...
	if result.Status == "error" {
		slog.Error("provider conversion failed", "group", "convert", "category", "fonts", "provider", result.Provider)
		stats.TrackError(c, "convert", "fonts", "CONVERT_FAILED")
...
func (h *Handler) Upload(c *gin.Context) {
	...
	if err != nil {
		slog.Error("upload conversion failed", "group", "convert", "category", "fonts", "error", err)
		stats.TrackError(c, "convert", "fonts", "CONVERT_ERROR")
	...
	if result.Status == "error" {
		slog.Error("provider upload conversion failed", "group", "convert", "category", "fonts", "provider", result.Provider)
		stats.TrackError(c, "convert", "fonts", "CONVERT_FAILED")
		response.ErrorWithCode(c, 500, "CONVERT_FAILED", "Conversion failed on the provider side. Please try again or use a different file.")
		return
	}
