...
func (h *Handler) Extract(c *gin.Context) {
	stats.Platform(c, "content", "twitter")
	var req Request
	...
	if cached, err := downloader.CacheGet[mediaresponse.TwitterResponse]("content", "twitter", req.URL); err == nil && cached != nil {
		for i, v := range cached.Download.Video {
			...
			cached.Download.Video[i].Server2 = downloader.GenerateServer2URL(
				h.appURL, h.streamSecret, downloader.CacheKey("content", "twitter", v.Original),
				v.Original, customTitle, "", "", ext, "content",
			)
		}
		if cached.Download.Audio != nil {
			...
			cached.Download.Audio.Server2 = downloader.GenerateServer2URL(
				h.appURL, h.streamSecret, downloader.CacheKey("content", "twitter", cached.Download.Audio.Original),
				cached.Download.Audio.Original, audioTitle, "", "", "mp3", "content",
			)
		}
	...
	if err != nil {
		slog.Error("extract failed", "group", "content", "platform", "twitter", "error", err)
		stats.TrackError(c, "content", "twitter", "EXTRACTION_FAILED")
	...
	for _, v := range result.Videos {
		...
		server2 := downloader.GenerateServer2URL(
			h.appURL, h.streamSecret, downloader.CacheKey("content", "twitter", v.URL),
			v.URL, customTitle, "", "", ext, "content",
		)
	...
	if result.AudioURL != "" {
		...
		audio = &mediaresponse.ContentAudio{
			...
			Server2: downloader.GenerateServer2URL(
				h.appURL, h.streamSecret, downloader.CacheKey("content", "twitter", result.AudioURL),
				result.AudioURL, audioTitle, "", "", audioExt, "content",
			),
		}
	}
