...
func (h *Handler) Extract(c *gin.Context) {
	...
	if !validator.IsValidURL(req.URL) || !validator.IsAllowedDomain(req.URL, "videb") {
		response.ErrorWithCode(c, 400, "INVALID_URL", "URL not supported for this endpoint.")
		return
	}

	cacheKey := downloader.CacheKey("vidhub", "videb", req.URL)

	if cached, err := downloader.CacheGet[mediaresponse.VidhubResponse]("vidhub", "videb", req.URL); err == nil && cached != nil {
	...
	if err != nil {
		slog.Error("extract failed", "group", "vidhub", "platform", "videb", "error", err)
		stats.TrackError(c, "vidhub", "videb", "EXTRACTION_FAILED")
	...
	downloader.CacheSet("vidhub", "videb", req.URL, &res)
