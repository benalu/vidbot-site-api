...
func NewHandler(downloadWorkerURL, downloadWorkerSecret, workerXORKey, appURL, streamSecret string) *Handler {
	return &Handler{
		service:              NewService(),
...
func (h *Handler) Extract(c *gin.Context) {
	...
	if !validator.IsValidURL(req.URL) || !validator.IsAllowedDomain(req.URL, "vidbos") {
		response.InvalidURLWarn(c, "vidhub", "vidbos", req.URL)
		return
	}

	cacheKey := downloader.CacheKey("vidhub", "vidbos", req.URL)

	if cached, err := downloader.CacheGet[mediaresponse.VidhubResponse]("vidhub", "vidbos", req.URL); err == nil && cached != nil {
	...
	if err != nil {
		response.Extraction(c, "vidhub", "vidbos", err)
		return
	}
	...
	downloader.CacheSet("vidhub", "vidbos", req.URL, &res)
